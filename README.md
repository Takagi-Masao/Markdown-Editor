...
- **双重保存机制**：既可保存 Markdown 文件也可以导出 PDF。在 Chrome/Edge 中「保存」会直接写回原文件（File System Access API），其余浏览器自动回退为下载保存。
- **草稿自动保存**：编辑内容自动保存到浏览器本地存储，刷新或重开页面时可一键恢复，避免意外丢失。
- **未保存提醒**：当前文件有未保存修改时，打开新文件（或拖入新文件）前会弹出「保存 / 不保存 / 取消」确认框，防止修改丢失。
