...
- **便携式部署**：将所有功能整合至双击即可使用的 `all-in-one.html`，无需额外配置。（随着项目变得更复杂，后续版本可能不再提供此整合后的文件，但仍会确保部署便捷性）
...
2. 使用本地服务器打开 `index.html`，如：
   ...
   或者直接双击 `all-in-one.html` 即可使用。当然，这个文件也可以通过本地服务器打开访问。编辑器内置了一段示例内容，包含标题、代码块、公式等，可直接体验或清空后自行编辑。
3. 编辑 Markdown 内容或者本地上传 Markdown 文件，右侧实时预览效果。
4. 点击「导出 PDF」按钮，在弹出的打印对话框中选择「另存为 PDF」即可保存。

## 📄 打印/导出 PDF 说明

- 打印输出包含浏览器自带的页眉页脚，且带有舒适的页面边距。
